...
<div class="flex justify-end mb-4">
    <a href="{{ route('organisator.events.create') }}"
        class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">Créer un Event</a>
</div>
    ...
    @foreach ($myEvents as $event)
        ...
        <div class="transition-all duration-150 flex w-full px-4 py-6 md:w-1/2 lg:w-1/3">
            ...
            <div
                ...
                <a href="{{ route('organisator.events.show', $event) }}" class="w-full">
                    <div class="md:flex-shrink-0">
                        <img src="{{ $event->getFirstMediaUrl('events') }}" alt="Picture"
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center flex-1">
                            <img class="object-cover h-10 rounded-full"
                                src="{{ $event->getFirstMediaUrl('events') }}" alt="Avatar" />
                            <div class="flex flex-col mx-2">
                                <a href="" class="font-semibold text-gray-700 hover:underline">
                                    {{ $event->name }}
                                </a>
                                {{-- <span class="mx-1 text-xs text-gray-600">{{ $event->created_at }}</span>
                            </div>
                        </div>
                        <p class="mt-1 text-xs text-gray-600">{{ $event-> }}</p> --}}
                    </div>
                    <div class="flex items-center justify-between px-4 py-2 overflow-hidden">
                        <!-- Bouton de modification -->
                        <a href="{{ route('organisator.events.edit', $event) }}"
                            class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Modifier</a>

                        <!-- Bouton de suppression -->
                        <form action="{{ route('organisator.events.destroy', $event) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
